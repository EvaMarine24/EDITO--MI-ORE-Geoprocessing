...
def upload_model_to_s3_bucket(dir_model):
    ...
    # setup s3 bucket
    S3_ENDPOINT_URL = os.environ["S3_ENDPOINT"] #S3 endpoint
    fs = s3fs.S3FileSystem(client_kwargs={'endpoint_url': S3_ENDPOINT_URL}) #endpoint url
    #onxia_user_name = os.environ["GIT_USER_NAME"] #TODO: convenient to have the username in a separate env var
    bucket_name = f"oidc-evamarine24"

    # remove dir_model_s3 folder if present, to prevent nested dir_model in dir_model
    dir_model_s3 = f's3/oidc-evamarine24/DFM_INPUT' #alternative option is {os.path.basename(dir_model)}
